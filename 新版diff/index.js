    ...
    default:
      switch (nextChildFlags) {
        ...
        default:
          ...
          outer: {
            while (preChildren[j].key === nextChildren[j].key) {
              patch(preChildren[j], nextChildren[j]);
              j++;
              if (j > nextEnd) {
                break outer;
              }
            }
            while (preChildren[preEnd].key === nextChildren[nextEnd].key) {
              patch(preChildren[preEnd], nextChildren[nextEnd]);
              preEnd--;
              nextEnd--;
            }
          }
          if (j > nextEnd) {
            for (let q = j; q <= preEnd; q++) {
              container.removeChild(preChildren[q].el);
            }
          } else if (j > preEnd) {
            for (let q = j; q <= nextEnd; q++) {
              mount(nextChildren[q], container, false, preChildren[j].el);
            }
          } else {
            const nextLeft = nextEnd - j + 1;
            const source = Array.from({ length: nextLeft }).fill(-1);
            const preStart = j;
            const nextStart = j;
            let preVNode;
            let nextVNode;
